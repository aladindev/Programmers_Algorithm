...
SELECT DISTINCT A.CAR_ID, DECODE(B.CAR_ID, null, '대여 가능', '대여중') AS AVAILABILITY
  FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY A
  LEFT JOIN
   (SELECT CAR_ID
      FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
     WHERE '20221016' BETWEEN TO_CHAR(START_DATE, 'YYYYMMDD') AND TO_CHAR(END_DATE, 'YYYYMMDD')) B
